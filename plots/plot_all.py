...
from plot_utils import (SEABORN_STYLE, powerlaw, fit_powerlaw,
                        apply_plot_style, group_output_name, label_for, is_train_split)
...
def _plot_ldds(group, exp_dir, out_dir, **_):
    ...
    with plt.style.context(SEABORN_STYLE):
        ...
        ax.set_xlim(1, x_lim)
        ax.set_ylim(0.6, 3.5)
        lgd = apply_plot_style(ax,
                               xlabel='Distance between words, d',
                               ylabel='Mutual Information, I(d)',
                               legend_loc='upper right', legend_ncol=5)
        plt.savefig(os.path.join(out_dir, out_name + '_ldds.png'),
    ...
    with plt.style.context(SEABORN_STYLE):
        ...
        for name in group:
            ...
            if is_train_split(name):
                amp, index, _ = fit_powerlaw(xdata, frequency, pinit=[0, -1.0], n_points=n_fit)
                train_index = index
        ...
        plt.savefig(os.path.join(out_dir, out_name + '_zipf.png'),
                    bbox_extra_artists=(lgd,), bbox_inches='tight')
        plt.clf()
        plt.close(fig)
...
def _plot_heaps(group, exp_dir, out_dir, **_):
    ...
    with plt.style.context(SEABORN_STYLE):
        ...
        for name in group:
            ...
            amp, index, _ = fit_powerlaw(xdata, vocab_sizes, pinit=[0, 0.5], n_points=n_fit)
            ax.loglog(xdata, vocab_sizes, label=f"{label_for(name, group)} : β={round(index, 3)}")
            plotted = True
        ...
        plt.savefig(os.path.join(out_dir, out_name + '_heaps.png'),
                    bbox_extra_artists=(lgd,), bbox_inches='tight')
        plt.clf()
        plt.close(fig)
...
def _plot_ebelings(group, exp_dir, out_dir, **_):
    ...
    with plt.style.context(SEABORN_STYLE):
        ...
        for name in group:
            ...
            amp, index, _ = fit_powerlaw(subseq_lens, variances, pinit=[0, -1.0], n_points=n_fit)
            ax.loglog(subseq_lens, variances, label=f"{label_for(name, group)} : η={round(index, 3)}")
            plotted = True
        ...
        lgd = apply_plot_style(ax,
                               ...
                               ylabel='Variance',
                               legend_loc='upper left')
        plt.savefig(os.path.join(out_dir, out_name + '_ebelings.png'),
                    bbox_extra_artists=(lgd,), bbox_inches='tight')
        plt.clf()
        plt.close(fig)
...
def _plot_taylors(group, exp_dir, out_dir, subseq_length=1000, **_):
    ...
    with plt.style.context(SEABORN_STYLE):
        ...
        for name in group:
            ...
            ax.loglog(means, powerlaw(means.astype(float), amp, index),
        ...
        lgd = apply_plot_style(ax,
                               ...
                               legend_loc='upper left')
        plt.savefig(os.path.join(out_dir, out_name + '_taylors.png'),
                    bbox_extra_artists=(lgd,), bbox_inches='tight')
        plt.clf()
        plt.close(fig)
...
def main():
    parser = argparse.ArgumentParser(description='Plot complex-system properties of LM datasets')
    parser.add_argument('--plots', nargs='+', default=list(PLOT_TYPES.keys()),
    ...
    parser.add_argument('--subseq_length', type=int, default=1000,
