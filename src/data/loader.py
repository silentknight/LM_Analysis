...
_REGISTRY = {
    ...
    # wikitext-2 cleaned
    "dataset/wiki2C/.full": (
        "wikitext-2 cleaned full",
        True,
        [("train", "trainC"), ("valid", "validC"), ("test", "testC")],
    ),
    ...
    "dataset/wiki2C/.valid1": (
        "wikitext-2 cleaned valid1",
        True,
        [("validC1", "validC1")],
    ),
    # OpenWebText (Skylion007 reproduction of GPT-2 training corpus)
    "dataset/openweb/.full": (
        "OpenWebText full",
        True,
        [("train", "train"), ("valid", "valid"), ("test", "test")],
    ),
    "dataset/openweb/.train": ("OpenWebText train", True, [("train", "train")]),
    "dataset/openweb/.valid": ("OpenWebText valid", True, [("valid", "valid")]),
    "dataset/openweb/.test": ("OpenWebText test", True, [("test", "test")]),
    # English Wikipedia (downloaded via dataset/wikipedia/download.py)
    "dataset/wikipedia/.full": (
        "Wikipedia full",
        True,
        [("train", "train"), ("valid", "valid"), ("test", "test")],
    ),
    "dataset/wikipedia/.train": ("Wikipedia train", True, [("train", "train")]),
    "dataset/wikipedia/.valid": ("Wikipedia valid", True, [("valid", "valid")]),
    "dataset/wikipedia/.test": ("Wikipedia test", True, [("test", "test")]),
    # wikitext-103
    "dataset/wiki103/.full": (
        "wikitext-103 full",
        True,
        [("train", "train"), ("valid", "valid"), ("test", "test")],
    ),
    ...
    "dataset/wiki103C/.full": (
        "wikitext-103 cleaned full",
        True,
        [("train", "trainC"), ("valid", "validC"), ("test", "testC")],
    ),
    "dataset/wiki103C/.train": (
        "wikitext-103 cleaned train",
        True,
        [("trainC", "trainC")],
    ),
    "dataset/wiki103C/.test": ("wikitext-103 cleaned test", True, [("testC", "testC")]),
    "dataset/wiki103C/.valid": (
        "wikitext-103 cleaned valid",
        True,
        [("validC", "validC")],
    ),
    # wikitext-103 raw (was incorrectly assigning to trainC/testC/validC;
    # fixed to train/test/valid)
    "dataset/wiki103R/.full": (
        "wikitext-103 raw full",
        True,
        [("train", "train"), ("valid", "valid"), ("test", "test")],
    ),
    ...
    # wikitext-19 cleaned
    "dataset/wiki19C/.full": (
        "wikitext-19 cleaned full",
        True,
        [("train", "train"), ("valid", "valid"), ("test", "test")],
    ),
    ...
    "dataset/wiki19L/.full": (
        "wikitext-19L Text8-Like full",
        True,
        [("train", "train"), ("valid", "valid"), ("test", "test")],
    ),
    "dataset/wiki19L/.train": (
        "wikitext-19L Text8-Like train",
        True,
        [("train", "train")],
    ),
    "dataset/wiki19L/.valid": (
        "wikitext-19L Text8-Like valid",
        True,
        [("valid", "valid")],
    ),
    "dataset/wiki19L/.test": ("wikitext-19L Text8-Like test", True, [("test", "test")]),
    # wikitext-19L-wor (Text8-Like without rare words)
    "dataset/wiki19L-wor/.full": (
        "wikitext-19L-wor full",
        True,
        [("train", "train"), ("valid", "valid"), ("test", "test")],
    ),
    "dataset/wiki19L-wor/.train": (
        "wikitext-19L-wor train",
        True,
        [("train", "train")],
    ),
    "dataset/wiki19L-wor/.valid": (
        "wikitext-19L-wor valid",
        True,
        [("valid", "valid")],
    ),
    ...
    "dataset/wiki2Resample/.full1": (
        "wikitext-2 Resampled 1 full",
        True,
        [("train", "trainR1"), ("valid", "validR1"), ("test", "testR1")],
    ),
    "dataset/wiki2Resample/.test1": (
        "wikitext-2 Resampled 1 test",
        True,
        [("test", "testR1")],
    ),
    "dataset/wiki2Resample/.train1": (
        "wikitext-2 Resampled 1 train",
        True,
        [("train", "trainR1")],
    ),
    "dataset/wiki2Resample/.valid1": (
        "wikitext-2 Resampled 1 valid",
        True,
        [("valid", "validR1")],
    ),
    "dataset/wiki2Resample/.full2": (
        "wikitext-2 Resampled 2 full",
        True,
        [("train", "trainR2"), ("valid", "validR2"), ("test", "testR2")],
    ),
    "dataset/wiki2Resample/.test2": (
        "wikitext-2 Resampled 2 test",
        True,
        [("test", "testR2")],
    ),
    "dataset/wiki2Resample/.train2": (
        "wikitext-2 Resampled 2 train",
        True,
        [("train", "trainR2")],
    ),
    "dataset/wiki2Resample/.valid2": (
        "wikitext-2 Resampled 2 valid",
        True,
        [("valid", "validR2")],
    ),
    "dataset/wiki2Resample/.full3": (
        "wikitext-2 Resampled 3 full",
        True,
        [("train", "trainR3"), ("valid", "validR3"), ("test", "testR3")],
    ),
    "dataset/wiki2Resample/.test3": (
        "wikitext-2 Resampled 3 test",
        True,
        [("test", "testR3")],
    ),
    "dataset/wiki2Resample/.train3": (
        "wikitext-2 Resampled 3 train",
        True,
        [("train", "trainR3")],
    ),
    "dataset/wiki2Resample/.valid3": (
        "wikitext-2 Resampled 3 valid",
        True,
        [("valid", "validR3")],
    ),
    "dataset/wiki2Resample/.full4": (
        "wikitext-2 Resampled 4 full",
        True,
        [("train", "trainR4"), ("valid", "validR4"), ("test", "testR4")],
    ),
    "dataset/wiki2Resample/.test4": (
        "wikitext-2 Resampled 4 test",
        True,
        [("test", "testR4")],
    ),
    "dataset/wiki2Resample/.train4": (
        "wikitext-2 Resampled 4 train",
        True,
        [("train", "trainR4")],
    ),
    "dataset/wiki2Resample/.valid4": (
        "wikitext-2 Resampled 4 valid",
        True,
        [("valid", "validR4")],
    ),
    # wikitext-2 samples (1–4)
    "dataset/wiki2Samples/.full1": (
        "wikitext-2 Samples 1 full",
        True,
        [("train", "train_1"), ("valid", "valid_1"), ("test", "test_1")],
    ),
    "dataset/wiki2Samples/.test1": (
        "wikitext-2 Samples 1 test",
        True,
        [("test", "test_1")],
    ),
    "dataset/wiki2Samples/.train1": (
        "wikitext-2 Samples 1 train",
        True,
        [("train", "train_1")],
    ),
    "dataset/wiki2Samples/.valid1": (
        "wikitext-2 Samples 1 valid",
        True,
        [("valid", "valid_1")],
    ),
    "dataset/wiki2Samples/.full2": (
        "wikitext-2 Samples 2 full",
        True,
        [("train", "train_2"), ("valid", "valid_2"), ("test", "test_2")],
    ),
    "dataset/wiki2Samples/.test2": (
        "wikitext-2 Samples 2 test",
        True,
        [("test", "test_2")],
    ),
    "dataset/wiki2Samples/.train2": (
        "wikitext-2 Samples 2 train",
        True,
        [("train", "train_2")],
    ),
    "dataset/wiki2Samples/.valid2": (
        "wikitext-2 Samples 2 valid",
        True,
        [("valid", "valid_2")],
    ),
    "dataset/wiki2Samples/.full3": (
        "wikitext-2 Samples 3 full",
        True,
        [("train", "train_3"), ("valid", "valid_3"), ("test", "test_3")],
    ),
    "dataset/wiki2Samples/.test3": (
        "wikitext-2 Samples 3 test",
        True,
        [("test", "test_3")],
    ),
    "dataset/wiki2Samples/.train3": (
        "wikitext-2 Samples 3 train",
        True,
        [("train", "train_3")],
    ),
    "dataset/wiki2Samples/.valid3": (
        "wikitext-2 Samples 3 valid",
        True,
        [("valid", "valid_3")],
    ),
    "dataset/wiki2Samples/.full4": (
        "wikitext-2 Samples 4 full",
        True,
        [("train", "train_4"), ("valid", "valid_4"), ("test", "test_4")],
    ),
    "dataset/wiki2Samples/.test4": (
        "wikitext-2 Samples 4 test",
        True,
        [("test", "test_4")],
    ),
    "dataset/wiki2Samples/.train4": (
        "wikitext-2 Samples 4 train",
        True,
        [("train", "train_4")],
    ),
    "dataset/wiki2Samples/.valid4": (
        "wikitext-2 Samples 4 valid",
        True,
        [("valid", "valid_4")],
    ),
    # wikitext-2 Homogenous (H1, H2, CH1, CH2)
    "dataset/wiki2Homogenous/.fullH1": (
        "wikitext-2 Homogenous 1 full",
        True,
        [("train", "trainH1"), ("valid", "validH1"), ("test", "testH1")],
    ),
    "dataset/wiki2Homogenous/.testH1": (
        "wikitext-2 Homogenous 1 test",
        True,
        [("test", "testH1")],
    ),
    "dataset/wiki2Homogenous/.trainH1": (
        "wikitext-2 Homogenous 1 train",
        True,
        [("train", "trainH1")],
    ),
    "dataset/wiki2Homogenous/.validH1": (
        "wikitext-2 Homogenous 1 valid",
        True,
        [("valid", "validH1")],
    ),
    "dataset/wiki2Homogenous/.fullH2": (
        "wikitext-2 Homogenous 2 full",
        True,
        [("train", "trainH2"), ("valid", "validH2"), ("test", "testH2")],
    ),
    "dataset/wiki2Homogenous/.testH2": (
        "wikitext-2 Homogenous 2 test",
        True,
        [("test", "testH2")],
    ),
    "dataset/wiki2Homogenous/.trainH2": (
        "wikitext-2 Homogenous 2 train",
        True,
        [("train", "trainH2")],
    ),
    "dataset/wiki2Homogenous/.validH2": (
        "wikitext-2 Homogenous 2 valid",
        True,
        [("valid", "validH2")],
    ),
    "dataset/wiki2Homogenous/.fullCH1": (
        "wikitext-2 Homogenous Cleaned 1 full",
        True,
        [("train", "trainCH1"), ("valid", "validCH1"), ("test", "testCH1")],
    ),
    "dataset/wiki2Homogenous/.testCH1": (
        "wikitext-2 Homogenous Cleaned 1 test",
        True,
        [("test", "testCH1")],
    ),
    "dataset/wiki2Homogenous/.trainCH1": (
        "wikitext-2 Homogenous Cleaned 1 train",
        True,
        [("train", "trainCH1")],
    ),
    "dataset/wiki2Homogenous/.validCH1": (
        "wikitext-2 Homogenous Cleaned 1 valid",
        True,
        [("valid", "validCH1")],
    ),
    "dataset/wiki2Homogenous/.fullCH2": (
        "wikitext-2 Homogenous Cleaned 2 full",
        True,
        [("train", "trainCH2"), ("valid", "validCH2"), ("test", "testCH2")],
    ),
    "dataset/wiki2Homogenous/.testCH2": (
        "wikitext-2 Homogenous Cleaned 2 test",
        True,
        [("test", "testCH2")],
    ),
    "dataset/wiki2Homogenous/.trainCH2": (
        "wikitext-2 Homogenous Cleaned 2 train",
        True,
        [("train", "trainCH2")],
    ),
    "dataset/wiki2Homogenous/.validCH2": (
        "wikitext-2 Homogenous Cleaned 2 valid",
        True,
        [("valid", "validCH2")],
    ),
    ...
    "dataset/hutter/text8-wo-r-2": (
        "text8 without rare words 2",
        False,
        [("full", None)],
    ),
    "dataset/hutter/text8-small-wo-r-2": (
        "text8 small without rare words 2",
        False,
        [("full", None)],
    ),
    "dataset/hutter/text8-small-wo-r-4": (
        "text8 small without rare words 4",
        False,
        [("full", None)],
    ),
    # text8w (subdirectory)
    "dataset/hutter/text8w/.full": (
        "text8w full",
        True,
        [("train", "train"), ("valid", "valid"), ("test", "test")],
    ),
    ...
    "dataset/hutter/text8w_small/.full": (
        "text8w small full",
        True,
        [("train", "train.txt"), ("valid", "valid.txt"), ("test", "test.txt")],
    ),
    "dataset/hutter/text8w_small/.train": (
        "text8w small train",
        True,
        [("train", "train.txt")],
    ),
    "dataset/hutter/text8w_small/.valid": (
        "text8w small valid",
        True,
        [("valid", "valid.txt")],
    ),
    "dataset/hutter/text8w_small/.test": (
        "text8w small test",
        True,
        [("test", "test.txt")],
    ),
    # ptb_text8 (subdirectory, .txt extensions)
    "dataset/ptb_text8/.full": (
        "ptb_text8 full",
        True,
        [("train", "train.txt"), ("valid", "valid.txt"), ("test", "test.txt")],
    ),
    ...
    "dataset/ptb_text8/train_small": (
        "ptb_text8 train small",
        False,
        [("train", None)],
    ),
}
...
class SequentialData(object):
    ...
    def add_data(self):
        ...
        self.averageLength = (
            self.averageLength * len(self.wordCountList) + len(self.__wordLine)
        ...
        with open(path, "r", encoding="utf-8") as f:
            for line in f:
                if self.ifwords == 1:
                    ...
                    # Replace <unk> with a sentinel that cannot appear as a real char
