...
class MyThread(threading.Thread):
    def __init__(
        self, d, overlap, log_type, method, data_array, line_length_list, total_length
    ):
        self.d = d
        self.Ni_X = 0
        self.Ni_Y = 0
    ...
    def run(self):
        ...
        if self.method == "standard":
            ...
            # Fancy indexing builds the correct layout without sparse matrices.
            ...
            pmi_data = lddcalc.getStandardPMI(
                ...
                P_X,
                P_Y,
                np.uint64(P_XY.data.size),
                np.uint64(P_X.size),
                np.uint64(P_Y.size),
                self.log_type,
            )
            self.pmi = sp.coo_matrix(
                (pmi_data, (P_XY.row, P_XY.col)), shape=P_XY.shape
            ).tocsc()
            self.Ni_X = Ni_X
            self.Ni_Y = Ni_Y
            self.Ni_XY = Ni_XY
...
class PointwiseMutualInformation(object):
    ...
    def calculate_PMI(self):
        ...
        try:
            ...
            print(
                self.directory + " does not exist or cannot be loaded; starting fresh."
            )
        ...
        try:
            with tqdm(
                total=total_d, initial=d - 1, desc="PMI", unit="d", dynamic_ncols=True
            ) as pbar:
                while d < max_distance and d <= self.cutoff and not end:
                    ...
                    for i in range(self.no_of_threads):
                        if thread[i].complete:
                            ...
                            break
                        np.savez_compressed(
                            os.path.join(self.directory, "marginals", str(d + i)),
                            Xi=thread[i].Xi,
                            Yi=thread[i].Yi,
                            Ni_X=thread[i].Ni_X,
                            Ni_Y=thread[i].Ni_Y,
                        ...
                        sp.save_npz(
                            os.path.join(self.directory, "Ni_XY", str(d + i)),
                            thread[i].Ni_XY,
                        )
                        sp.save_npz(
                            os.path.join(self.directory, "pmi", str(d + i)),
                            thread[i].pmi,
                        )
