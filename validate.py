...
def cmp_dataset_ids(new_path, ref_path):
    ...
    if short == long_[:len(short)] and len(long_) - len(short) <= 1:
        return True, f"{len(short):,} tokens match (extra trailing: {len(long_)-len(short)})"
...
def cmp_zipf(new_path, ref_path):
    ...
    parts = []
    if not ids_ok:  parts.append("ids differ")
    if not freq_ok: parts.append("frequency differs")
...
def cmp_ldd_csv(new_path, ref_path, abs_tol=1e-10, **_):
    def load(p):
        with open(p, newline='') as f:
            r = csv.reader(f)
            next(r); next(r)
            return {int(row[0]): [float(x) for x in row[1:5]] for row in r if len(row) >= 5}
    new = load(new_path)
    ref = load(ref_path)
    if set(new) != set(ref):
    ...
    max_abs = max(
        abs(nv - rv)
        for d in new for nv, rv in zip(new[d], ref[d])
    )
    ok = max_abs <= abs_tol
...
def cmp_heaps(new_path, ref_path, **_):
    ...
    short, long_ = (new, ref) if len(new) <= len(ref) else (ref, new)
    if short == long_[:len(short)] and len(long_) - len(short) <= 1:
        return True, f"{len(short):,} values match (extra trailing: {len(long_)-len(short)})"
    diffs = sum(a != b for a, b in zip(new, ref))
...
def cmp_taylors(new_path, ref_path, abs_tol=1e-6, rel_tol=0.01, **_):
    ...
    return ok, f"max_abs={max_abs:.2e}  max_rel={max_rel:.2e}  tol=abs:{abs_tol:.0e}/rel:{rel_tol:.0%}"


def cmp_ebelings(new_path, ref_path, abs_tol=1e-4, rel_tol=0.01, **_):
    def load(p):
        with open(p) as f:
            return {int(t.split(':')[0]): float(t.split(':')[1])
                    for t in f.read().split(',') if ':' in t}
    new = load(new_path)
    ref = load(ref_path)
    if set(new) != set(ref):
...
def manifest(sp):
    return [
        ("datasetInIDs", f"experiments/datasetInIDs/{sp}.csv",      "datasetInIDs", f"{sp}.csv",          cmp_dataset_ids),
        ("zipf",         f"experiments/zipf/{sp}.npz",               "zipf",         f"{sp}.npz",           cmp_zipf),
        ("ldds",         f"experiments/ldds/{sp}.csv",               "ldds",         f"{sp}.csv",           cmp_ldd_csv),
        ("heaps",        f"experiments/heaps/{sp}_heaps.csv",        "heaps",        f"{sp}_heaps.csv",     cmp_heaps),
        ("taylors",      f"experiments/taylors/{sp}_taylors.csv",    "taylors",      f"{sp}_taylors.csv",   cmp_taylors),
        ("ebelings",     f"experiments/ebelings/{sp}_ebelings.csv",  "ebelings",     f"{sp}_ebelings.csv",  cmp_ebelings),
        ("recurrence",   f"experiments/recurrence/{sp}.npz",         "recurrence",   f"{sp}.npz",           cmp_recurrence),
    ]
...
def cmd_save(args):
    ...
    if args.run_cmd:
        ...
        print(f"  saved  run command → cmd.txt")
...
def cmd_check(args):
    ...
    for name, src, subdir, fname, cmp_fn in manifest(sp):
        ...
        if not os.path.exists(src):
            ...
            continue
        try:
            ok, detail = cmp_fn(src, ref_path, **{k: v for k, v in tol.get(name, {}).items()
...
def main():
    parser = argparse.ArgumentParser(description="Baseline validation for LM_Analysis outputs")
    sub = parser.add_subparsers(dest="command", required=True)

    p_save = sub.add_parser("save", help="Save current experiments/ outputs as baseline")
    p_save.add_argument("--save_path", required=True)
    p_save.add_argument("--run_cmd", default="",
                        help="Command string to store and replay during 'check'")

    p_check = sub.add_parser("check", help="Re-run analysis and compare against baseline")
    p_check.add_argument("--save_path", required=True)
    p_check.add_argument("--skip_run", action="store_true",
